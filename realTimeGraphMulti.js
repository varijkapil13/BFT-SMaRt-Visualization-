...
function realTimeChartMulti() {
	...
	var chart = function (s) {
		...
		function refresh() {
			...
			updateSel
				...
				.attr("cx", function (d) {
					...
					switch (getTagName(this)) {
						case "circle":
							//console.log(d)
							timeForSender = Math.round(x(d.time));
							timeForReceiver = Math.round(x(d.time2));
				...
				.attr("cy", function (d) {
					...
					switch (getTagName(this)) {
						case "circle":
							senderNode = Math.round(y(d.category));
							receiverNode = Math.round(y(d.category2));
							retVal = y(d.category);
							break;
						default:
					}
			...
			// data removed from this file. refer to the original example to see how the circles and rectangles were drawn.
			...
			for (incer = 0; incer < data.length; incer++) {
				var lineData1 = [{
						"x": data[incer].time,
						"y": data[incer].category
					},
					{
						"x": data[incer].time2,
						"y": data[incer].category2
						}
					];

				lineArray.push(lineData1);
				colorArray.push(data[incer].lineColor)
				var lineSelNav1 = barG.selectAll(".line")
					.data(lineArray);
				...
				lineSelNav1.attr("d", lineFunction1);
