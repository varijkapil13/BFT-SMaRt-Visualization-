...
public final class Acceptor {
    ...
    public final void processMessage(ConsensusMessage msg) {
        ...
        long senttime;
        if (msg.messageSentFromServerTime == 0){
        	senttime = msg.systemMessageSentTime;
        }
        else {
        	senttime = msg.messageSentFromServerTime;
        }
        
        switch (msg.getType()){
            case MessageFactory.PROPOSE:{
            	
            	producerObject.consensusMessageReceived("propose", msg.getSender(), me, msg.getEpoch(), senttime);
                    proposeReceived(epoch, msg);
            }break;
            case MessageFactory.WRITE:{
            	producerObject.consensusMessageReceived("write", msg.getSender(), me, msg.getEpoch(), senttime);
                    writeReceived(epoch, msg.getSender(), msg.getValue());
            }break;
            case MessageFactory.ACCEPT:{
            	producerObject.consensusMessageReceived("accept", msg.getSender(), me, msg.getEpoch(), senttime);
